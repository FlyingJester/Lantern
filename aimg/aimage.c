...
AIMG_EXPORT void AImg_CloneImage(struct AImg_Image *to, const struct AImg_Image *from){
...
AIMG_EXPORT void AImg_DestroyImage(struct AImg_Image *that){
    free(that->pixels);
    that->pixels = NULL;
    that->w = that->h = 0;
}

AIMG_EXPORT void AImg_CreateImage(struct AImg_Image *that, unsigned w, unsigned h){
    that->w = w;
    that->h = h;
...
AIMG_EXPORT void AImg_Blit(const struct AImg_Image *src, struct AImg_Image *dst, int x, int y){
    assert(src);
    assert(dst);
...
AIMG_EXPORT void AImg_SetPixel(struct AImg_Image *to, int x, int y, uint32_t color){
    if(x < 0 || y < 0 || x >= to->w || y >= to->h)
        return;
    AImg_Pixel(to, x, y)[0] = color;
}

AIMG_EXPORT uint32_t AImg_GetPixel(struct AImg_Image *to, int x, int y){
    if(x < 0 || y < 0 || x >= to->w || y >= to->h)
        return 0;
...
AIMG_EXPORT uint32_t *AImg_Pixel(struct AImg_Image *to, int x, int y){
    return (uint32_t *)AImg_PixelConst(to, x, y);
}

AIMG_EXPORT const uint32_t *AImg_PixelConst(const struct AImg_Image *to, int x, int y){
    return to->pixels + x + (y * to->w);
}

/* 0xFF00FFFF is yellow. That is all. */

AIMG_EXPORT uint32_t AImg_RGBAToRaw(uint8_t r, uint8_t g, uint8_t b, uint8_t a){
    return (a << 24) | (b << 16) | (g << 8) | (r);
}

AIMG_EXPORT void AImg_RawToRGBA(uint32_t rgba, uint8_t *r, uint8_t *g, uint8_t *b, uint8_t *a){
    r[0] = rgba & 0xFF;
    rgba >>= 8;
    g[0] = rgba & 0xFF;
    rgba >>= 8;
...
AIMG_EXPORT uint8_t AImg_RawToR(uint32_t rgba){
    return rgba & 0xFF;
}

AIMG_EXPORT uint8_t AImg_RawToG(uint32_t rgba){
    return (rgba >> 8) & 0xFF;
}

AIMG_EXPORT uint8_t AImg_RawToB(uint32_t rgba){
    return (rgba >> 16) & 0xFF;
}

AIMG_EXPORT uint8_t AImg_RawToA(uint32_t rgba){
    return (rgba >> 24) & 0xFF;
}
...
AIMG_EXPORT void AImg_FlipImageVertically(const struct AImg_Image *from, struct AImg_Image *to){
...
AIMG_EXPORT uint32_t AImg_RGBABlend(uint8_t src_r, uint8_t src_g, uint8_t src_b, uint8_t src_a, uint8_t dst_r, uint8_t dst_g, uint8_t dst_b, uint8_t dst_a){
...
AIMG_EXPORT unsigned AImg_LoadAuto(struct AImg_Image *to, const char *path){
...
AIMG_EXPORT unsigned AImg_SaveAuto(const struct AImg_Image *from, const char *path){
    const char * const end = path + strlen(path), *str = end;
    while(str[0] != '.'){
        if(str==path)
            return AIMG_LOADPNG_NFORMAT;
